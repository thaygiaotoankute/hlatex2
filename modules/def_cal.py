            ...
            if '.' in str(Fraction(m).limit_denominator()):
                ...
                if '/' in str(Fraction(m).limit_denominator()):
                    if float(num) > 0:
                        ...
                        b = str(m)[s + 1:]
                        if len(b) > 4:
                            # This line has an issue, using 'n' which is not defined
...
def get_tu_phanso(num):
    ...
    if '/' in a:
        s = a.index('/')
        tu = int(a[:s])
        # This line has an issue - mau is not defined
        # Removing it since it's not being used
        # return (tu, mau)
        return tu
    else:
        tu = num
        # Same issue - mau is not defined
        # return (tu, mau)
        return tu
